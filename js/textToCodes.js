...
function hexToDecimal(input){
  //.reverse() is required for appropriate multiplication in  the conversion
  let hexNums = input.split('').reverse();
  console.log(hexNums);
  let altCode = 0;
  for (let i = 0; i < hexNums.length; i++) {
    //convert only alpha characters to number, leave string nums alone
    if(Number.isInteger(hexNums[i]*1) !== true){
      //only work on the single element, not the entire array for the .map
      let notNumArr = [];
      notNumArr.push(hexNums[i]);
      //an alpha character can be converted to a num using UTF 16 index
      let alphaToNum = notNumArr.map(x => x.charCodeAt()) - 55;
      //utf 16 to digit
      let altToNum = String.fromCharCode(alphaToNum);
      hexNums.splice(i,1,alphaToNum);
    }
    //convert string nums  to true numbers
    let hexArrInt = parseInt(hexNums[i],10);
    hexNums.splice(i,1,hexArrInt);
    //looped addition for the true alt code
    altCode = altCode + (hexNums[i]*1)*(16**i);
  }
  return altCode;
}
